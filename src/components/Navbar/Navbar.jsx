...
const Navbar = () => {
  ...
  const delaySuggestions =  setTimeout(() => {
      if (search.trim() !== "") {
        getSuggestionsAPI(search).then(res => setSuggestions(res.data) ).catch(
          err => console.log(err)
        )
      }else{
        setSuggestions([])
  ...
  return (
    ...
    <div className='align_center'>
        <h1 className='navbar_heading' > <NavLink to="/"> SneakAttack</NavLink>
        </h1>
      ...
      // this is responsible for clearing the suggestion tag after selecting the product.
      onClick={() => {
        setSearch("");
        setSuggestions([]);
      }}
